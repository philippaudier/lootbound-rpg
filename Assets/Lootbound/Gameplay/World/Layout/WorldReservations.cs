...
namespace Lootbound.Gameplay.World.Layout
{
    ...
    /// <summary>
    /// A reserved location for a landmark, attached to a host node.
    /// This is not a graph node - it represents a position for distinctive terrain features.
    /// </summary>
    public sealed class LandmarkReservation
    {
        /// <summary>
        /// Deterministic ID: "landmark_{seed}_{index}".
        /// </summary>
        public string ReservationId { get; }
        ...
        public static string GenerateId(int seed, int index)
        {
            return $"landmark_{seed}_{index}";
        }

        public override string ToString()
        {
            return $"LandmarkReservation[{ReservationId}] host={HostNodeId} ring={Ring}";
        }
    }
}
