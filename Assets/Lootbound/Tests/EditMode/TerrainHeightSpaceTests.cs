...
{
    /// <summary>
    /// Regression tests for the terrain height-space contract: the
    /// NormalizedHeightMap is the map actually applied to the Unity Terrain, so
    /// every world-space height query must read it (otherwise spawned content
    /// floats above or sinks below the real ground). Since slice T2 there is no
    /// global normalization pass any more; these tests cover the sampling
    /// contract only.
    /// </summary>
    public class TerrainHeightSpaceTests
    {
        ...
        [Test]
        public void SampleHeightAtWorld_UsesNormalizedMap_NotRawNoise()
        {
            var context = CreateContext();

            // Raw and normalized maps deliberately diverge: the query must read
            // the normalized (applied) map, which is the real ground.
            ...
            Assert.AreEqual(0.8f * TERRAIN_HEIGHT, sampled, EPSILON,
                "SampleHeightAtWorld must read the normalized map (the real ground), not the raw noise");
        }
